...
public:
    ...
    status_t startPreviewCallbacks(android::CameraParameters &params, CameraBuffer *buffers, uint32_t *offsets, int fd, uint32_t length, uint32_t count);
    ...
    status_t initSharedVideoBuffers(CameraBuffer *buffers, uint32_t *offsets, int fd, uint32_t length, uint32_t count, CameraBuffer *vidBufs);
...
private:
    ...
    status_t allocImageBufs(unsigned int width, unsigned int height, uint32_t length,
